...
if (process.env.REACT_APP_API_URL === 'production') {
  // In production, construct the URL based on the current domain
  // Assumes the Netlify function is at the same domain under /.netlify/functions/api
  API_URL = `${window.location.origin}/.netlify/functions/api`;
} else {
  // In development, use the local server
  API_URL = 'http://localhost:5000/.netlify/functions/api'; // Correct path for local testing
}
